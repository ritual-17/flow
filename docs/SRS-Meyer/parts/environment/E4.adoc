...
*	*User is able to download and install applications on their operating system.*:	As we cannot be there to install this program on every computer it will run on, we assume that the user is able to install it by themselves on their supported operating system (Windows, macOS, or Linux) provided they are given an installer or package file.
