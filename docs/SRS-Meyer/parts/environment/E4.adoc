...
*	*User is able to download and install applications on their operating system.*:	As installation cannot be performed on every target computer directly, the user is assumed to be able to install the application independently on their supported operating system (Windows, macOS, or Linux) provided they are given an installer or package file.
...
*	*Device being run on has proper libraries or user can install them when provided with redists.*: If the application uses other libraries, those libraries must be installed for it to work. If the user can install the application, it is assumed the user can also install required libraries. These libraries need to be provided either as a list or as redistributable files. 
