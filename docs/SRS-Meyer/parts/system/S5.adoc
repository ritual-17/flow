...
| F216 | Pan and zoom canvas | **High** | G.4 explicitly names panning as a core feature; for a keyboard-driven infinite canvas, navigation is essential to basic use.
| F217 | Save and load notes from local storage | **High** | Data persistence and continuity.  
| F243 | Export notes (PDF, PNG, Markdown) | **Medium** | Important for sharing, but not needed for MVP.  
| F219 | Undo/redo editing actions | **High** | In a modal keyboard-driven editor where actions are rapid and irreversible, undo/redo is critical for preventing data loss and user frustration.
...
| NF211 | Usability: clean and intuitive editor layout to minimize user confusion | **High** | Critical for learnability and user satisfaction.
| NF212 | Responsiveness: canvas interactions (e.g., drag, resize, type) shall respond with less than 50ms latency | **High** | Essential for a fluid editing experience.  
...
* Text and diagram creation/editing — F211–F216, NF211–NF214
* Local save/load, autosave, and undo/redo — F217, F219, NF215
* Command interface and discoverability — NF217

===== Medium Priority
Enhancements that improve usability and reach:

* Export features — F243
...
| F243 | NF215 | Exporting notes depends on reliable file handling.
...
| NF211 | F211–F216 | A clean and intuitive layout underpins all editing interactions.
| NF212 | F213–F216 | Canvas responsiveness (≤50ms) is required for smooth text entry, shape manipulation, and navigation.
