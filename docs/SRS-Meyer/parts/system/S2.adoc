...
. [[NF214]] **Discoverability**: The editor shall provide a discoverable command palette and keyboard shortcut reference accessible from within the editor at any time. (NF214)
. [[NF217]] **Visual consistency**: The visual appearance of UI elements (shapes, text, canvas layout) shall remain consistent across sessions and screen sizes. (NF217)

---

==== (S.2.2) Canvas Manager

===== Functional Requirements

. [[F216]] **Reposition and resize elements**: The Canvas Manager shall allow text boxes and shapes to be freely moved and resized within the canvas. (F216)
. [[F217]] **Canvas navigation**: The Canvas Manager shall allow users to pan across the workspace in all directions. (F217)

_Note: IDs F216 and F217 retain their original numbering to avoid cascading changes to the Hazard Analysis and VnV Plan, which already reference these IDs._

===== Non-Functional Requirements

. [[NF212]] **Responsiveness**: Canvas interactions (e.g., drag, resize, pan) shall respond with less than 50ms latency. (NF212)

_Note: NF212 retains its original ID for the same reason._

---

==== (S.2.3) Shortcut Command Processor
...
. [[NF221]] **Low latency**: Command input shall be interpreted and dispatched within 30ms of user keypress. This measures command execution time only; for commands that also trigger a canvas update, the end-to-end input-to-visual-feedback latency is governed by <<NF212>> (≤50ms), which is the binding constraint. (NF221)
...
. [[NF231]] **Performance**: Rendering shall maintain a minimum of 60 frames per second for at least 95% of the time during continuous user interaction (typing, dragging, panning) on a canvas containing up to 100 mixed elements (text and shapes). (60fps is targeted because it is the baseline for smooth animation in web and multimedia applications) (NF231)
. [[NF232]] **Consistency**: Shapes and text shall retain consistent proportions and alignment across devices. (NF232)
. [[NF233]] **Scalability**: Rendering shall handle up to 500 on-canvas elements without major performance degradation. (500 elements 
was chosen due to typical usage scenarios) (NF233)

---

==== (S.2.5) Document Management

===== Functional Requirements

. [[F241]] **File operations**: The system shall allow users to save, open, rename, and delete notes. (F241)
. [[F242]] **Autosave**: The system shall autosave changes at regular intervals (≤60 seconds as that is 
the typical interval for most applications) (F242)
...
. [[NF242]] **Storage efficiency**: A note containing up to 1000 words of text and 10 shapes shall occupy less than 100KB on disk. (NF242)
. [[NF243]] **Compatibility**: Exported documents shall remain visually consistent across standard PDF viewers. (NF243)
. [[NF215]] **File integrity**: Save and export operations shall be atomic; a failed or interrupted operation shall not result in a partially written or corrupted file. (NF215)

---

==== (S.2.6) Configuration and Customization
...
. [[NF216]] **Privacy**: The application shall not initiate any background network traffic; all user data shall remain on the local device. (NF216)

---

==== (S.2.7) Collaboration and Export
...
==== (S.2.8) Basis for Design and Requirements Stability

_Note: This section is informational and does not correspond to a system component. It supplements sections S.2.1–S.2.7 by making component inputs/outputs explicit and classifying requirements by likelihood of change, so an external team could design, build, and test from this document._
...
| Canvas Manager | Pointer events (drag, resize, pan), canvas model | Repositioned/resized element states, updated viewport position
