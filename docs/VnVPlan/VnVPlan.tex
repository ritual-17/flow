...
09/03/2026 & Hussain, Ethan & Update Unit Testing\\
03/04/2026 & Hussain & Final Review and Edits\\
\bottomrule
\end{tabularx}
...
1. \textbf{Editing and Canvas Operations} – covering note creation, editing, and manipulation (SRS F211–F216). \\
2. \textbf{File Management and Persistence} – covering saving, loading, and export functions (SRS F217–F243).
...
Input: User inserts a text box, enters text, and edits the text content.

Output:
The text box appears correctly on the canvas, entered text is displayed correctly, and edits are reflected immediately without visual issues or lag.

Test Case Derivation:
Expected behavior is defined by F213, which requires that users can create text boxes and modify their contents after creation.
The expected value is that the displayed text exactly matches the user input after each edit operation.

How test will be performed: Automated functional test using simulated keyboard and mouse inputs. The test will insert a text box, enter a predefined string, edit part of the string, and compare the final rendered text against the expected result.
...
Initial State: Empty Linux (Ubuntu 22.04), Windows (11), Mac computer (macOS Sonoma).

Input/Condition: Creating and opening note, adding shapes and text.

Output/Result: Shapes or text are shown on screen without crashes, missing UI elements, or save/load issues across all three operating systems.

How test will be performed: The tester will create a note, add shapes and text, save, close, and reopen the note on each operating system.
...
Output/Result: Shapes or text are shown on screen when short cuts are used and shortcut produces correct action.

How test will be performed: The tester will open a note and then attempt to add a note and a shape and a text box via the corresponding short cut. All keyboard shortcuts will be tested once.
...
Output/Results: Shapes or text are shown on screen when short cuts are used and shortcut produces correct action.

How test will be performed: See (test-F217, F243). We then compare shape position, size, color, text content, formatting, and overall layout between the saved and reopened note. Additionally, we will attempt to close a note without saving and check it the autosave catches it
...
Output/Results: No network traffic is detected during note creation, editing, saving, or closing.

How test will be performed: Code review on all parts that may use the network data, making sure each can't run in the background. This may include modules related to autosave, update checking, or third party libraries. 
This is preferable to capturing network information during runtime as it is more deterministic and can be done earlier in the development process. However, if there are any doubts about potential network traffic, we can also use a network monitoring tool to verify that no traffic is being sent during typical application usage.
...
The unit tests focuses on modules that implement the core logic of Flow, particularly those
...
involved when shapes are created, modified, or interpreted as part of command execution.
Because these modules primarily support the functionality of higher-level modules, their
behavior is verified through tests that manipulate geometry or execute commands rather than
...
Finally, modules primarily responsible for user interaction and rendering, such as the
Display Interface Module (M1) and Input Interface Module (M2), were considered lower
priority for unit testing. These modules largely depend on external UI frameworks and
graphical behavior that is better evaluated through integration and usability testing
rather than isolated unit tests. As a result, the current unit testing effort focuses on
verifying the correctness of the system's internal logic and code quality whereas
higher-level testing methods, such as a usability test, are used to evaluate user-facing
behavior.
...
\subsubsection{File Interface Module (M3)}
...
\textbf{This functionality is not implemented yet and therefore not tested yet. It is 
prioritized for after the Final Demo as it is not critical for the core functionality of the app.}
...
\subsubsection{Commands Parser Module (M6)}
...
Initial State: Editor in Normal mode

Input: 'i' (switch to Insert mode)

Output: 'enterInsertMode' (a valid command name)

Test Case Derivation:

This output is derived from the expected behavior of the command parser, which
    should correctly interpret the 'i' command as a mode switch to Insert mode.

How test will be performed:

Unit test that can be run manually or automatically as part of the CI pipeline.

\item{M6.T2\\}

Type: Unit, Automatic.

Initial State: Editor in Insert mode, Document is empty

Input: 'c' (create a circle)

Output: 'createCircle' (a valid command name)

Test Case Derivation:

This output is derived from the expected behavior of the command parser, which
    should correctly interpret the 'c' command as a shape creation command to create a circle.
...
\subsubsection{Geometry State Module (M11)}
...
Input: createNewDocument('testDoc')

Output: A new document object with the name 'testDoc' and an empty geometry state.

Test Case Derivation:
This output is derived from the expected behavior of the geometry state module, which
    should correctly initialize a new document with the specified name and an empty state.
...
\subsubsection{Geometry State Mutator Module (M12)}
...
Initial State: An empty document 'document', cursor position (via Editor state)

Input: createCircle(document, editor)

Output: A new circle shape is added to the document's geometry state at the cursor position.

Test Case Derivation:
This output is derived from the expected behavior of the geometry state mutator module, which
    should correctly update the document's geometry state to include new shapes when they are added.
...
Initial State: A document with a circle, current selection on the circle (via Editor state)

Input: translateSelectionDown(document, editor), defined TRANSLATE\_AMOUNT

Output: The circle's position in the document's geometry state is updated to be TRANSLATE\_AMOUNT units down from its original position.

Test Case Derivation:
This output is derived from the expected behavior of the geometry state mutator module, which
    should correctly update the position of selected shapes in the document's geometry state when translation commands are executed.
...
\subsubsection{Undo Redo Module (M13)}
...
Initial State: A document with no history.

Input: undo(initialDocument)

Output: initialDocument (no change since there is no history)

Test Case Derivation:
This output is derived from the expected behavior of the undo redo module, which
    should not alter the document state when an undo command is issued with no history.
...
Initial State: A document with one action in history (e.g., a circle was created).

Input: undo(document)

Output: document with the circle removed (reverted to the state before the circle was created).

Test Case Derivation:
This output is derived from the expected behavior of the undo redo module, which
    should correctly revert the document state to the previous state when an undo command is issued.
...
Initial State: A document with an undid action in the redo history (e.g., a circle was created, then undone).

Input: redo(document)

Output: document with the circle re-added (reverted to the state before the undo was executed).

Test Case Derivation:
This output is derived from the expected behavior of the undo redo module, which
    should correctly reapply the last undone action to the document state when a redo command is issued.
...
Unit testing has been used to verify the behavior of individual modules in isolation before integration testing occurs. 
Automated unit tests are implemented for the Command Parser Module (M6), Geometry State Module (M11), Geometry State 
Mutator Module (M12), and Undo/Redo Module (M13). These tests validate expected behavior for command parsing, document 
state changes, object manipulation, and history tracking functionality. Test cases include both the sample cases 
outlined in the VnV Plan and additional edge cases identified during development.

All implemented unit tests are currently passing and are executed through the project's automated testing framework. 
Unit test implementations are located within the test/unit directory of the develop branch. The File Interface Module 
(M3) does not yet have unit tests because its functionality has not yet been implemented
...
1. Do you use any of the following in your day-to-day work flows? (Click all that apply) \\
- Vim/Emacs \\
- A window manager \\
- Some reliance on keyboard shortcuts (e.g. Ctrl-c, Ctrl-z, Ctrl-v) \\
- Heavy reliance on keyboard shortcuts (i.e. beyond simple shortcuts such as Ctrl-c, Ctrl-z) \\
- Some other keyboard driven tool (fill in below) \\
- Other: 

2. Do you have experience writing notes involving mathematical equations with any of the following tools? 
(Click all that apply) \\
- Typst \\
- LaTeX \\
- Google Docs \\
- OneNote \\
- Some other tool (fill in below) \\

3. What is/has been your method for taking notes involving diagrams and/or math equations? 
(e.g. pen and paper, Google docs, OneNote)

4. On a scale of 1-5, how well does the interface communicate which "Mode" (e.g., Insert, Normal, Visual) 
you are currently in?

5. Which statement best describes your feelings towards learning Flow \\
- I intend to learn to use it upon its release \\
- I could see myself learning it \\
- I probably wouldn't want to learn it \\
- I wouldn't learn it \\

6. How much do you agree with the following statement? \\
If I used Flow every day for a week, I feel I could take notes faster here than with my current solution.

7. How much do you agree with the following statement? \\
If I used Flow every day for a month, I feel I could take notes faster here than with my current solution.

8. Which statement best described your feelings when using Flow \\
- The system was extremely intuitive. I was able to predict some keybinds before using them. \\
- The system was intuitive. Once I learned the commands, most of them made sense and stuck with me. \\
- The system was somewhat intuitive. Some commands made sense when I learned them, but they didn't stick with me. \\
- The system was not intuitive. It was difficult to memorize commands and overall usage was confusing. \\

9. How likely would you be to use Flow over the following tool for taking notes? \\
Pen and Paper

10. How likely would you be to use Flow over the following tool for taking notes? \\
Tablet

11. How likely would you be to use Flow over the following tool for taking notes? \\
Online docs (e.g. Google Docs, Word, Obsidian)

12. How likely would you be to use Flow over the following tool for taking notes? \\
Online notes (e.g. OneNote, Notability)

13. How likely would you be to use Flow over the following tool for taking notes? \\
Online drawing (e.g. draw.io, Google drawing)

14. What is the biggest 'deal-breaker' currently preventing you from switching to this full-time? \\
- Lack of features/maturity \\
